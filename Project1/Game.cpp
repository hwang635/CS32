	...
	for (;;)
	{
		...
		else if (playerMove.size() == 1)
		{
			if (tolower(playerMove[0]) == 'x')
				return player->dropPoisonVial();
			else if (tolower(playerMove[0] == 'h')) { //wrong number of turns, counts h as turn + not sure if not moving vamp = turn
...
void Game::play()
{
	...
	while (!player->isDead() && m_arena->vampireCount() > 0)
	{
		...
		if (player->isDead())
			break;
		if(msg != "") //If the command is h (history), which doesn't count as a turn
