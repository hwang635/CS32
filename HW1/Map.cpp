...
void Map::swap(Map& other) {
	int smallerSize = 0;
	//Find the smaller size of this, other map
	if (other.size() <= this->size())
		smallerSize = other.size();
	else
		smallerSize = this->size();

	//Go through maps and swap items up to the end of the smaller map
	for (int i = 0; i < smallerSize; i++) {
		KeyType tempKey = items[i].m_key;
		ValueType tempVal = items[i].m_value;
		this->items[i].m_key = other.items[i].m_key;
		this->items[i].m_value = other.items[i].m_value;
		other.items[i].m_key = tempKey;
		other.items[i].m_value = tempVal;
	}

	//If this map has more items than other map, put remaining items from this map into other map and delete items from this map
	if (this->size() > other.size()) {
		//Loop until bigger map has same amount of items as smaller items (all of big maps items have been put in small map)
		//Insert first unswapped item from big map into smaller map and erase swapped item in big map until all items swapped 
		while (this->size() > smallerSize) {
			other.insert(items[smallerSize].m_key, items[smallerSize].m_value);
			this->erase(items[smallerSize].m_key);
		}
	}
	else {
		while (other.size() > smallerSize) {
			this->insert(other.items[smallerSize].m_key, other.items[smallerSize].m_value);
			other.erase(other.items[smallerSize].m_key);
		}
	}
